...
exports.sendFriendReq = async(req, res) => {
    try{
        ...
        if(newRequest){
            const user1 = await User.findByIdAndUpdate(userId, { $push: {friends: newRequest._id}}, {new: true});
            const user2 = await User.findByIdAndUpdate(friendId, { $push: {friends: newRequest._id}}, { new: true});
...
exports.cancelFriendReq =  async(req, res) => {
    try {
        ...
        const pendingRequest = await Friendship.findOne({ user1: userId, user2: friendId });
        if (!pendingRequest) {
        ...
        if (!updatedUser1 || !updatedUser2) {
            return res.status(500).json({
                success: false,
                message: "Error updating users' friends arrays while cancelling friend request.",
            });
        }
...
exports.rejectFriendReq =  async(req, res) => {
    try {
        ...
        // Checking if the friend request exists
        const pendingRequest = await Friendship.findOne({ user1: friendId, user2: userId });
        if (!pendingRequest) {
            return res.status(404).json({
                success: false, 
        ...
        if (!updatedUser1 || !updatedUser2) {
            return res.status(500).json({
                success: false,
                message: "Error updating users' friends arrays while rejecting friend request.",
            });
        }
